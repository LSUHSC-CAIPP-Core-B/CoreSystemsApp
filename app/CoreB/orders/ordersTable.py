        ...
        if len(Uinputs) != 0:
            ...
            if Uinputs[1] != "" and Uinputs[0] == "":
                data = search_utils.sort_searched_data(
                    Uinputs, columns_to_check, 99, SqlData, order_by
                )
                ...
                data = search_utils.sort_searched_data(
                    Uinputs, columns_to_check, 50, SqlData, order_by
                )

                # If no match is found displays empty row
                if data.empty:
                    data = db_utils.toDataframe(
                        "Select * FROM CoreB_Order WHERE `Project ID` = 'N/A';",
