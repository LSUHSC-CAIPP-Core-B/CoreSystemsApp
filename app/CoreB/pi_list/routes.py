...
def information():
    ...
    if request.method == "GET":
        ...
        if data.empty:
            ...
            filtered_data["PI full name"] = filtered_data["PI full name"].replace(
                "_", " "
            )
    ...
    if request.method == "GET":
        ...
        if cached_data is None:
            ...
            dataFrame = db_utils.toDataframe(
                "Select * FROM pi_info;", "db_config/CoreB.json"
            )
...
def update():
    ...
    if request.method == "GET":
        ...
        df = db_utils.toDataframe(
            query, "db_config/CoreB.json", params=(old_pi_full_name,)
        )
        ...
        filters = session.get(
            "pi_list_filters", {"pi_name": "", "dept": "", "sort": "Original"}
        )
...
def downloadCSV():
    ...
    return send_file(
        csv_io, mimetype="text/csv", as_attachment=True, download_name="PI List.csv"
    )
