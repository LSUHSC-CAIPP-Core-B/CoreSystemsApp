    ...
    **Response:**
    ...
    if request.method == "GET":
        ...
        if cached_data is None:
            ...
            df = db_utils.toDataframe(
                "SELECT * FROM Mouse_Stock WHERE Genotype != 'N/A';",
            ...
            df.rename(
                columns={
                    ...
                    "Times_Back_Crossed": "Times Back Crossed",
                    "MTA_Required": "MTA Required",
                },
                inplace=True,
            )
...
def addMouse():
    ...
    **Response:**
    ...
    if request.method == "POST":
        ...
        has_empty_value = any(
            value == "" or value is None for value in inputData.values()
        )
...
def changeMouse():
    ...
    **Response:**
    ...
    if request.method == "GET":
        ...
        # use to prevent user from caching pages
        response = make_response(
            render_template("CoreC/change_mouse.html", fields=data, pkey=primary_key)
        )
        response.headers["Cache-Control"] = (
            "no-cache, no-store, must-revalidate"  # HTTP 1.1.
        )
...
def downloadCSV():
    ...
    if (
        num_rows == 0
    ):  # If the database had no rows (Empty) then send a message to the user
        flash(" No records to download")
        return redirect(url_for("mouse.mouse"))

    elif num_rows > 0:  # If the message is not empty then download CSV file
        csv_io = mouseTable.download_CSV(
            saved_data=saved_data, dropCol=["Stock_ID", "user_id"]
        )
