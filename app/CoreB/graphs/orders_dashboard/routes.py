...
def build_dashboard(df, html_out_path):
    ...
    # Detect columns
    date_col = find_col(
        ["order date", "date", "order_date", "created", "timestamp", "datetime"], cols
    )
    product_col = find_col(
        ["product", "item", "sku", "product name", "product_name"], cols
    )
    category_col = find_col(["category", "type", "segment", "class"], cols)
    region_col = find_col(
        ["region", "country", "state", "city", "zone", "market", "area"], cols
    )
    status_col = find_col(
        ["status", "order status", "fulfillment", "shipment status"], cols
    )
    ...
    mask_biorender = df.astype(str).apply(
    ...
    if date_col and df["_order_day"].notna().any():
        ...
        plt.xlabel("Date")
        plt.ylabel("Orders")
        path = os.path.join(out_dir, "orders_over_time.png")
        save_fig(path)
    ...
    if category_col:
        ...
        if len(cat_counts) > 0:
            ...
            plt.title(f"Orders by {category_col}")
            plt.xlabel("Orders")
            path = os.path.join(out_dir, "orders_by_category.png")
            save_fig(path)
    ...
    if product_col:
        prod = (
            df.groupby(product_col)["_revenue"]
            .sum()
    ...
    if region_col:
        ...
        if len(reg) > 0:
            ...
            plt.title(f"Orders by {region_col}")
            plt.xlabel("Orders")
            path = os.path.join(out_dir, "orders_by_region.png")
            save_fig(path)
    ...
    if status_col:
        ...
        if len(st) > 0:
            ...
            path = os.path.join(out_dir, "status_breakdown.png")
            save_fig(path)
    ...
    if pi_col:
        ...
        if len(top_pis) > 0:
            ...
            plt.title("Top PIs by Number of Orders")
            plt.xlabel("Orders")
            path = os.path.join(out_dir, "top_pis_by_orders.png")
            save_fig(path)
    ...
    if funding_col:
        ...
        if len(funding_counts) > 0:
            ...
            path = os.path.join(out_dir, "funding_breakdown.png")
            save_fig(path)
            add_card(
                "Funding Source Breakdown (NIH, NSF, DoD, NASA, Other)",
                to_base64_img(path),
            )
        ...
        if len(cobre_counts) > 0:
            ...
            plt.pie(
                cobre_counts.values,
                labels=cobre_counts.index,
                autopct="%1.1f%%",
                startangle=90,
            )
            plt.title("Grant Sources: COBRE vs Non-COBRE")
            path = os.path.join(out_dir, "grant_cobre_breakdown.png")
            save_fig(path)
    ...
    if service_col:
        ...
        if len(service_counts) > 0:
            ...
            plt.xlabel("Orders")
            path = os.path.join(out_dir, "orders_by_service_type_bar.png")
            save_fig(path)
