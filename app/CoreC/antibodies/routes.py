...
def antibodies():
    ...
    **Response:**
    ...
    if request.method == "POST":
        ...
        with app.app_context():
            cache1.set(
                "cached_dataframe", data, timeout=3600
        ...
        if cached_data is None:
            ...
            if current_user.is_admin:
                dataFrame = db_utils.toDataframe(
                    "SELECT Stock_ID, Box_Name, Company_name, Catalog_Num, Target_Name, Target_Species, Fluorophore, Clone_Name, Isotype, Size, Concentration, DATE_FORMAT(Expiration_Date, '%m/%d/%Y') AS Expiration_Date, Titration, Volume, Cost FROM Antibodies_Stock ORDER BY Target_Name;",
                ...
                dataFrame = db_utils.toDataframe(
                    "SELECT Stock_ID, Company_name, Catalog_Num, Target_Name, Target_Species, Fluorophore, Clone_Name, Isotype FROM Antibodies_Stock WHERE Included = 1 ORDER BY Target_Name;",
                    "db_config/CoreC.json",
                )
                dataFrame.rename(
                    columns={
...
def addAntibody():
    ...
    **Response:**
    ...
    if request.method == "GET":
        ...
        # use to prevent user from caching pages
        response = make_response(
            render_template("CoreC/add_antibody.html", fields=data)
        )
        response.headers["Cache-Control"] = (
            "no-cache, no-store, must-revalidate"  # HTTP 1.1.
        )
...
def changeAntibody():
    ...
    **Response:**
    ...
    if request.method == "POST":
        ...
        if included == "Not Valid":
            return redirect(
                url_for("antibodies.changeAntibody", primaryKey=primary_key)
            )
        ...
        # use to prevent user from caching pages
        response = make_response(
            redirect(url_for("antibodies.antibodies", page=current_page))
        )
        response.headers["Cache-Control"] = (
            "no-cache, no-store, must-revalidate"  # HTTP 1.1.
        )
...
def downloadCSV():
    ...
    return send_file(
        csv_io, mimetype="text/csv", as_attachment=True, download_name="Antibodies.csv"
    )
