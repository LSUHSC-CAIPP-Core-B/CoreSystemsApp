...
def panels():
    if request.method == "POST":
        ...
        dataFrame.rename(
            columns={"Panel_name": "Panel", "antibody_num": "Number of Antibodies"},
            inplace=True,
        )
    ...
    if request.method == "GET":
        ...
        panels_df = db_utils.toDataframe(
            "SELECT Panel_name, Panel_table_name from predefined_panels;",
    ...
    response = make_response(
        render_template(
            "CoreC/predefined_Antibody_Panels.html",
...
def addPanel():
    if request.method == "POST":
        ...
        db_utils.execute(
            name_query, "db_config/CoreC.json", params=(panel_name, db_name)
        )
...
def deletePanel():
    if request.method == "GET":
        ...
        name_df = db_utils.toDataframe(
            panel_table_name_query, "db_config/CoreC.json", params=(panel_name,)
        )
...
def panel_details():
    ...
    if request.method == "GET":
        ...
        table_name_dict = search_utils.search_data(
            [panel_name], columns_to_check=columns, threshold=90, SqlData=panels_df
        )
        ...
        if not table_name.empty:
            ...
            if current_user.is_admin:
                dataFrame = db_utils.toDataframe(
                    f"SELECT a.Stock_ID, a.Box_Name, a.Company_name, a.Catalog_Num, a.Target_Name, a.Target_Species, a.Fluorophore, a.Clone_Name, a.Isotype, a.Size, a.Concentration, DATE_FORMAT(a.Expiration_Date, '%m/%d/%Y') AS Expiration_Date,  a.Titration,  a.Cost FROM Antibodies_Stock a JOIN `{names}` m ON a.Stock_ID = m.stock_id WHERE a.Included = 1 ORDER BY a.Target_Name;",
                    "db_config/CoreC.json",
                )
                dataFrame.rename(
                    columns={
                ...
                dataFrame = db_utils.toDataframe(
                    f"SELECT a.Stock_ID, a.Company_name, a.Catalog_Num, a.Target_Name, a.Target_Species, a.Fluorophore, a.Clone_Name, a.Isotype FROM Antibodies_Stock a JOIN `{names}` m ON a.Stock_ID = m.stock_id WHERE a.Included = 1 ORDER BY a.Target_Name;",
                    "db_config/CoreC.json",
                )
                dataFrame.rename(
                    columns={
        ...
        if not data:
            if current_user.is_admin:
                flash(f"{panel_name}")
                return redirect(
                    url_for("panels.addPanelAntibody", Panel_Name=panel_name)
                )
            else:
...
def addPanelAntibody():
    ...
    if request.method == "POST":
        ...
        results = search_utils.search_data(
            [catalog_num], columns_to_check=["Catalog_Num"], threshold=99, SqlData=df
        )
        ...
        db_utils.execute(
            insert_Antibody_query, "db_config/CoreC.json", params=(results.iloc[0, 0],)
        )
    ...
    if request.method == "GET":
        ...
        response = make_response(
            render_template(
                "CoreC/add_panel_antibody.html", fields=data, Panel_Name=panel_name
            )
        )
        response.headers["Cache-Control"] = (
            "no-cache, no-store, must-revalidate"  # HTTP 1.1.
...
def deletePanelAntibody():
    if request.method == "POST":
        ...
        """
        db_utils.execute(
            delete_antibody_query, "db_config/CoreC.json", params=(primaryKey,)
        )
...
def changePanelName():
    if request.method == "POST":
        ...
        id_dataframe = db_utils.toDataframe(
            changeIDQuery, "db_config/CoreC.json", params=(panel_name,)
        )
        ...
        oldDBNameQuery = (
            "SELECT Panel_table_name from predefined_panels where Panel_Name = %s;"
        )
        Old_Panel_Df = db_utils.toDataframe(
            oldDBNameQuery, "db_config/CoreC.json", params=(panel_name,)
        )
    ...
    if request.method == "GET":
        ...
        response = make_response(
            render_template(
                "CoreC/change_panel_name.html", fields=data, Panel_Name=panel_name
            )
        )
        response.headers["Cache-Control"] = (
            "no-cache, no-store, must-revalidate"  # HTTP 1.1.
